...
class SingleInvoices extends Component
{
    ...
    public function edit($id)
    {
        ...
        $this->section_id = DB::table('section_tanslations')->where('id', $single_invoice->section_id)->first()->name;
        ...
        if ($this->type == 1) {
            ...
            try {
                ...
                if ($this->updateMode) {
                    ...
                    $single_invoices->section_id = DB::table('section_tanslations')->where('name', $this->section_id)->first()->section_id;
                    ...
                    // حفظ في جدول الفواتير {Store}
                    $single_invoices = new Invoice();

                    $single_invoices->invoice_type = 1; /// فاتورة مفردة
                    $single_invoices->invoice_date = date('Y-m-d');
                    $single_invoices->patient_id = $this->patient_id;
                    $single_invoices->doctor_id = $this->doctor_id;
                    $single_invoices->section_id = DB::table('section_tanslations')->where('name', $this->section_id)->first()->section_id;
